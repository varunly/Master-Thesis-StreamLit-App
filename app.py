...
# Complete Version with Energy File Upload, ND Correction & All Plots
# ==============================================================
...
# ==============================================================
# ENERGY CALIBRATION FUNCTIONS (FILE UPLOAD PARSER)
# ==============================================================
def parse_energy_file(file_content: str, file_type: str, file_bytes: bytes = None) -> Dict[float, Dict]:
    """
    Parse energy calibration file
    Format:
    Row 1: QS levels (200, 190, 180, ...)
    Rows 2-11: Energy measurements (10 readings)
    Row 13: Average (optional)
    """
    energy_map = {}
    
    st.info("🔄 Parsing energy calibration file...")
    ...
    try:
        ...
        if file_bytes:
            ...
            try:
                ...
                st.success(f"✅ Loaded Excel file: {df.shape[0]} rows × {df.shape[1]} columns")
            except:
                # Try CSV/TSV
                try:
                    ...
                    for sep in ['\t', ',', ';', ' ']:
                        df_temp = pd.read_csv(StringIO(content), sep=sep, header=None, engine='python')
                        if df_temp.shape[1] > 1:
                            df = df_temp
                            st.success(f"✅ Loaded text file (sep='{sep}'): {df.shape[0]} rows × {df.shape[1]} columns")
                            break
        ...
        # Show preview
        with st.expander("📄 File Preview (first 15 rows)", expanded=False):
        ...
        for val in qs_row:
            try:
                if pd.notna(val):
                    num = float(val)
                    # QS levels should be between 100-500
                    if 100 <= num <= 500:
                        qs_levels.append(num)
            except:
                continue
        
        if len(qs_levels) == 0:
            st.error("❌ No valid QS levels found in first row")
            st.info("Expected: numeric values like 200, 190, 180, 170, ...")
            return {}
        ...
        # Extract energy readings from rows 2-11 (indices 1-10)
        data_start_row = 1
        data_end_row = min(11, df.shape[0])  # Rows 2-11 = indices 1-10
                ...
                if col_idx < len(row):
                    try:
                        ...
                        if pd.notna(val):
                            ...
                            # Convert J to mJ if value is very small (< 0.01)
                            if energy < 0.01:
                                energy *= 1000
        ...
        st.success(f"✅ Extracted {readings_count} energy readings from rows 2-11")
        ...
        st.success(f"✅ Successfully parsed {len(final_map)} QS-Energy pairs")
...
st.markdown("**Lorentzian fitting • OD correction • Energy file upload • Complete analysis suite**")
...
with col2:
    st.subheader("⚡ Energy Calibration File")
    
    with st.expander("📋 Expected File Format", expanded=False):
        st.markdown("""
        ### File Structure
        
        **Row 1**: QS levels (200, 190, 180, 170, ...)
        **Rows 2-11**: Energy measurements (10 readings per QS)
        **Row 13**: Average (optional, calculated automatically)
        ...
        6.53E-06    3.07E-05    5.05E-05    1.18E-04
        ... (7 more measurement rows)
        
        0.000008    0.000025    0.000058    0.000122
        ```
        ...
        ### Notes:
        - Values in Joules (J) are auto-converted to mJ
        - Scientific notation (e.g., 7.48E-06) is supported
        - Only first 11 rows are used for calculations
        """)
    
    energy_file = st.file_uploader(
        "Upload energy calibration file",
        type=['xlsx', 'xls', 'csv', 'txt', 'tsv'],
        help="Excel or text file with QS levels and energy measurements"
    )
    ...
    if energy_file:
        try:
            ...
            st.error(f"❌ Error loading energy file: {str(e)}")
            energy_map = {}
    else:
        st.info("💡 Upload an energy calibration file to enable energy-based analysis")
...
if uploaded_files:
    ...
    if energy_map:
        st.info(f"⚡ **Energy calibration active** ({len(energy_map)} QS levels loaded)")
    ...
    with st.expander("📖 Instructions"):
        ...
        2. **Upload energy calibration file** (right column, optional)
        ...
        **Auto-extracts:**
        ...
        ### Energy Calibration File Format
        ...
        **Supported formats**: Excel (.xlsx), CSV, TSV, TXT
        
        ### Plots Generated
        - **Individual Spectra**: Lorentzian fits with OD correction
        - **Combined Spectra**: All overlaid
        - **Threshold Dashboard**: 4-panel analysis
        - **Energy vs Wavelength**: Peak shifts (if energy file uploaded)
        - **Energy vs Intensity**: Growth curves (if energy file uploaded)
